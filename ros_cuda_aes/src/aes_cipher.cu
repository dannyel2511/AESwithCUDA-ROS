...
int *cipher_main(byte *data, int data_size, byte **ciphered_data, byte *cip_key) {
//int main() {
    ...
    file_in      =  (byte*)          malloc(data_size * byte_size);
    ...
    *file_out_size = (*blocks) * 16 + 1;
    ...
    memcpy(key, cip_key, 16 *byte_size);
    ...
    //print_state_cpu(file_out);
    //write_file(out_file_name, file_out, file_out_size);
    memcpy(*ciphered_data, file_out, *file_out_size);

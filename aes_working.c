...
/*Realiza una rotacion circular de un byte a la derecha*/
...
/*Funciones auxiliares para el InverseMixColumns*/
byte mul_9(byte a)     { return M9[a];  }
byte mul_11(byte a) { return M11[a]; }
byte mul_13(byte a) { return M13[a]; }
byte mul_14(byte a) { return M14[a]; }
...
/*------------Descifrado----------------*/
/*Realiza rotacion de bytes a la derecha*/
...
/*Utiliza una matriz RS-box para realizar una substitución byte a byte del bloque del estado.*/
void inverseSubBytes() {
    byte i, j;
    for(i=0;i<4;i++) 
        for(j=0;j<4;j++) 
            state[j * 4 + i] = getRSBox(state[j * 4 + i]);
}

/*Substitución inversa de MixColumns que usa aritmética de campos finitos sobre GF(2^^8).*/
...
//Funcion principal que descompone el archivo entero en bloques y lo cifra/descifra
//option = 1, indica cifrado
//option = 0, indica descifrado
void cipher_or_decipher_file(byte *f_i, byte *f_o, byte option) {
    ...
    printf("Expandiendo la llave...\n\r");
    keyExpansion(); //Expandir la llave
    
    if(option==1)                 
        printf("Comienza lectura de archivos y cifrado.\n\r");
    else
        printf("Comienza lectura de archivos y descifrado.\n\r");                            
        
     //Ruta completa del archivo
     if(option==1)
         sprintf(file_route, "files/%s", f_i);//Ruta completa del archivo para cifrar
     else                                                            
         sprintf(file_route, "files/%s.aes", f_i);//Ruta completa del archivo para descifrar                                                                     
     ...
     if(option==1 && size%16!=0)//Solo aplica para cifrar, ya que para descifrar los bloques siempre son exactos
         blocks++; //Hay un bloque que no llega a 16 bytes
     
     //Crear archivo de salida  (escritura)
     if(option==1)
         sprintf(file_route, "files/%s.aes", f_o);//Ruta completa del archivo a cifrar
     else
         sprintf(file_route, "files/%s", f_o);//Ruta completa del archivo a descifrar
     ...
     if((file_out = fopen(file_route, "wb")) == NULL) {
         ...
         if(option==1) {
             ...
             //Revisar los bytes de padding
             //Eso es en el primer byte, se lee, luego se ignora y comienza el proceso normal
             fread(buffer, sizeof(byte), 1, file_in); //leer archivo
             padding=buffer[0];
         }
         printf("Trabajando...\n");   
         ...
         for(block=1;block<=blocks;block++) {
             ...
             //Decidir si es cifrado o descifrado              
             if(option==1) {  
             ...
             //Revisar el padding                                                                         
             if(block == blocks && option == 0) {//Ultimo bloque del descifrado
                 //Escribir solo los elementos necesarios al archivo                    
                 fwrite(buffer, sizeof(byte), 16-padding,file_out);                    
             }                                                                
             else fwrite(buffer, sizeof(byte), 16, file_out); //Escribir el bloque cifrado al archivo
         ...
         if(option==1) printf("Tiempo del cifrado: %lu.%lu s\n\r", time_encry/10, time_encry%10);
         else printf("Tiempo del descifrado: %lu.%lu s\n\r", time_encry/10, time_encry%10);            
...
int main() {
    ...
    printf("Decipher process started\n\r");
    printf("Selecciono descifrar.\n\r");
    
    file_name = "test.jpg";
    out_file_name = "testde.jpg";
    
    read_key_from_file();//Solicitar que se introduzca la llave
    
    //Funcion para comenzar descifrado del archivo
    cipher_or_decipher_file(file_name, out_file_name,0);                                    
